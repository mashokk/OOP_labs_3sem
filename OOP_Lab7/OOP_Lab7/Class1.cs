...
namespace OOP_Lab7
{
    ...
    public class Sweet : TastyTreat, ICook
    {
        ...
        public Sweet(string shape, string type, Weight weight)
        {
            ...
            this.Type = type;
            this.Weight = weight;
            /*if (shape.Length < 1 || shape.Length > 20)
                throw new ShapeException("Ошибка! Неккоректно введена форма:", shape); // че то
            else
                this.Shape = shape;
            if (type.Length < 1 || type.Length > 20)
                throw new ShapeException("Ошибка! Неккоректно введен тип:", type);
            else
                this.Type = type;*/
        }
    ...
    public class Cookie : TastyTreat, ICandyAssembling
    {
        ...
        public Cookie(string shape, string composition, Weight weight, int gram)
        {
            ...
            this.Weight = weight;
            //this.Gram = gram;
            /*if (shape.Length = "круглая" || shape.Length = "квадратная")
                throw new ShapeException("Ошибка! Неккоректно введена форма:", shape); // че то
            else
                this.Shape = shape;
            if (composition.Length < 1 || composition.Length > 20)
                throw new ShapeException("Ошибка! Неккоректно введен тип:", composition);
            else
                this.Composition = composition;
            this.Weight = weight;*/
            if (gram < 0 || gram > 9999)
                throw new GramException("Ошибка! Неккоректно введена граммовка:", gram);
            else
                this.Gram = gram;
        }
    ...
    public class Oven //духовка
    {
        public bool TryCooking()
        {
            ...
            Console.WriteLine("Попытка завести духовку: " + result);
            return result;
        }
    }
}
